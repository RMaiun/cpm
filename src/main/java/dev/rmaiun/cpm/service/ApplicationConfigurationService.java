...
import static dev.rmaiun.cpm.utils.RoleTypeMapper.authRoleToDomainTypes;
...
public class ApplicationConfigurationService {
  ...
  public EmptyDto processAppConfig(String user, ApplicationConfigurationDto dto) {
    ...
    var groupDtos = dto.domains().stream().flatMap(this::directConfigsToGroupRoles).toList();
  ...
  private Stream<GroupRoleDto> directConfigsToGroupRoles(DomainConfigurationDto domCfg) {
    return domCfg.directConfiguration().entrySet().stream()
        .map(e -> new GroupRoleDto(e.getKey(), domCfg.code(), authRoleToDomainTypes(e.getValue())));
  }
}
