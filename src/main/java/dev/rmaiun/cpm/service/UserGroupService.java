...
public class UserGroupService {
  ...
  public boolean checkUserCanWriteToDomain(String app, String domain, String user) {
    var foundGroups =
        groupRepository.listGroupAssignedToDomainByUserRole(app, domain, Set.of(WRITER), user);
  ...
  public void createMissingGroups(Application app, List<GroupRoleDto> groupRoles) {
    groupRoles.forEach(
        dto -> {
          var foundGroup =
              groupRepository
                  .findByAppCode(dto.group(), app.code())
                  .orElseGet(() -> createGroup(app, dto));
          bindGroupToDomainRoles(app, dto, foundGroup);
        });
  }

  public Map<String, List<String>> assignUserToGroup(
      Application app, Map<String, List<String>> groupUsers) {
    var appGroups =
        groupRepository.listByApp(app.code()).stream()
            .collect(toMap(BusinessGroup::code, BusinessGroup::id));
    var absentGroups =
        groupUsers.keySet().stream().filter(g -> isNull(appGroups.get(g))).collect(toSet());
