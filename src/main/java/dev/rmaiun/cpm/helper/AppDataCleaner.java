...
public class AppDataCleaner {
  ...
  public AppDataCleaner(DomainRepository domainRepository, DomainToDomainRepository domain2domainRepository, GroupRepository groupRepository,
  ...
  public void cleanAppData(Application app) {
    ...
    var groups = groupRepository.listByApp(app.code()).stream()
        .filter(g -> !List.of(Constants.APP_MANAGERS_GROUP, Constants.APP_OWNERS_GROUP).contains(g.code()))
